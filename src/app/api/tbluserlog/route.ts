...
export async function DELETE(
    ...
    prisma.tbluserlog.delete({
  where: {
    logID : id, // Replace with the actual ID of the user to delete
  },
})
 ...
 export async function GET(request: NextRequest  )
   
  {
   /* const session = request.cookies.get("session")?.value;
    ...
    const carts =makeSerializable( await prisma.tbluserlog.findMany())
  ...
  const getModel = async (id ) => {   
       const feed = await prisma.tbluserlog.findFirst({  where:
        { 
          AND:
          [
          {logID :parseInt(id)},
  ...
  export async function POST(request: NextRequest) {
    try{
     ...
     // console.log(customerID+" "+model.modelID+" "+model.price)
      const t=await prisma.tbluserlog.create({
            data: {
